    ...
    if(page_id==0 && html[6].length>0){
        ...
        $.each(html[6], function(e, l){
            ...
            if(blank!=undefined)
                blank = " target='_blank' ";
            if(url!=undefined)
                slideshow+="<a "+fancy+blank+"href='"+url+"'><li><img width='729' height='286' src='"+text+"' /></li></a>";
            else
                slideshow+="<li><img width='729' height='286' src='"+text+"' /></li>";
        });
        slideshow+="</ul>";
...
function get_pageContent(id, pageName, pageContent){
    ...
    $(pageContent.responseXML).find('page[id="'+id+'"]').each(function() {
        ...
        //                        is_blank[e] = $(this).attr('target');
        });
                
    });
	
    //console.log(pdfDownload);
    //content = [header, copy, mainImage, imageCaption, pdfDownload, pageTitle, slides, urls, is_fancy, is_blank];
