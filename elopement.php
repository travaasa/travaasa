<?php
$xml = simplexml_load_file('xml/copydeck-elopement.xml');
if ($_GET['pageId']){
	$pageId = $_GET['pageId'];
} else {
	$pageId = 2;
...
$count = 0;
$html = "";
foreach ($xml->elopement->secondaryPages->page as $section) {
...
foreach ($xml->elopement->secondaryPages->page as $section) {
	if ($section[secondaryNav] == "true"){
		...
		if ($count != 0){
			$html .= '<li class="'.$parentNav.$childNav.'"><a href="/elopement/'.$section->meta->slug.'" id="'.$section[id].'" class="' .$selected .'" rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
		} else {
			$html .= '<li class="top"><a href="/elopement/'.$section->meta->slug.'" id="'.$section[id].'" class="'.$selected.'" rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
		}
		$count++;
...
$countPfd = 0;
//$pdfDownload = array();
foreach ($xml->elopement->secondaryPages->page as $page) {
