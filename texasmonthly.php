<?php
$xml = simplexml_load_file('xml/copydeck-texasmonthly.xml');
if ($_GET['pageId']){
	$pageId = $_GET['pageId'];
} else {
	$pageId = 2;
...
$count = 0;
$html = "";
foreach ($xml->texasmonthly->secondaryPages->page as $section) {
...
foreach ($xml->texasmonthly->secondaryPages->page as $section) {
	if ($section[secondaryNav] == "true"){
		...
		if ($count != 0){
			$html .= '<li class="'.$parentNav.$childNav.'"><a href="/texasmonthly/'.$section->meta->slug.'" id="'.$section[id].'" class="' .$selected .'" rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
		} else {
			$html .= '<li class="top"><a href="/texasmonthly/'.$section->meta->slug.'" id="'.$section[id].'" class="'.$selected.'" rel="/'.$section->meta->slug.'">'.$title.'<img src="/images/nav/icon_selected-arrow.png" /></a></li>';
		}
		$count++;
...
$countPfd = 0;
//$pdfDownload = array();
foreach ($xml->texasmonthly->secondaryPages->page as $page) {
			...
			<div class="content_wrapper">
				<div id="header_frame">
					...
					<div id="reservation_cta">
						...
						<div id="book-austin"><a href="https://gc.synxis.com/rez.aspx?tps=fml&adult=1&step=1&hotel=28064&shell=fAUSTC&chain=10237&template=fAUSTC&avcurrency=USD" target="_blank" onclick="_gaq.push(['_link', 'https://gc.synxis.com/rez.aspx?tps=fml&adult=1&step=1&hotel=28064&shell=fAUSTC&chain=10237&template=fAUSTC&avcurrency=USD']); return false;">Book Austin</a></div>
